...
((char*) nodes_old)));
         ...
         // Adjust the pointers to the active node lists!
         if (queue_first[1])
         {
             node* queue_old = queue_first[1];
             queue_first[1] = (node*) ((char*)queue_first[1] + (((char*)
nodes) - ((char*) nodes_old)));
             node* queue = queue_first[1];

             while (queue->next != queue_old)
             {
                 queue_old = queue->next;
                 queue->next = (node*) ((char*)queue->next + (((char*)
nodes) - ((char*) nodes_old)));
                 queue = queue->next;
             }
         }
         ...
         if (queue_last[1])
         {
             node* queue_old = queue_last[1];
             queue_last[1] = (node*) ((char*)queue_last[1] + (((char*)
nodes) - ((char*) nodes_old)));
             node* queue = queue_last[1];
